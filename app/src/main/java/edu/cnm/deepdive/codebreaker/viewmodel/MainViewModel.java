...
public class MainViewModel extends AndroidViewModel implements LifecycleObserver {
  ...
  public void startGame() {
    ...
    pending.add(
        repository.newGame(POOL, codeLength, rng)
            ...
            .subscribe(game::postValue, throwable::postValue)
    );
  }
  ...
  public void restartGame() {
    ...
    int guessCount = this.game.getValue().getGuessCount();
    throwable.setValue(null);
    pending.add(
        repository.restartGame(game).doOnComplete(() -> {
          guess.postValue(null);
          solved.postValue(false);
          previousGuessCount += guessCount;
        }).subscribe(() -> this.game.setValue(game), throwable::postValue)
    );
  }
