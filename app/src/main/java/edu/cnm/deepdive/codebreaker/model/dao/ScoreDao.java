...
public interface ScoreDao {
  ...
  @Query(value = "SELECT * FROM Score WHERE code_length = :codeLength ORDER BY guess_count ASC LIMIT :recordCount")
  LiveData<List<Score>> selectBest(int codeLength, int recordCount);

  @Query(value = "SELECT code_length, AVG(guess_count) AS average_guess_count FROM Score GROUP BY code_length")
