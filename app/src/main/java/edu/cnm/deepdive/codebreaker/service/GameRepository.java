...
public class GameRepository {
  ...
  public Completable save(Score score) {
    return Completable.fromSingle(scoreDao.insert(score)).subscribeOn(Schedulers.io());
  }
  ...
  public Completable restartGame(Game game) {
    return Completable.fromAction(game::restart).subscribeOn(Schedulers.computation());
  }

  public Single<Guess> guess(Game game, String text) {
    return Single.fromCallable(() -> game.guess(text)).subscribeOn(Schedulers.computation());
  }
