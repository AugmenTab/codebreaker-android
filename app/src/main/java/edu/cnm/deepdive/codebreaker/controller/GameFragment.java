...
public class GameFragment extends Fragment implements InputFilter {
  ...
  @Override
  public CharSequence filter(CharSequence source, int sourceStart, int sourceEnd, Spanned dest,
      int destStart, int destEnd) {
  ...
  private void setupMaps() {
    ...
    codeCharacters = getString(R.string.color_codes).chars()
        .mapToObj((value) -> (char) value).toArray(Character[]::new);
  ...
  private void setupViews() {
    ...
    for (int i = 0; i < maxCodeLength; i++) {
      Spinner spinner = (Spinner) inflater.inflate
          (R.layout.swatch_spinner, binding.guessControls, false);
